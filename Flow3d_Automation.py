...
def runProcessing():
    ...
    # ------------------------------------All Code below is auto generated by Trace--------------------------------------#
    ...
    renderView1.CameraPosition = [-0.015, -0.015, 0.187741]
    renderView1.CameraFocalPoint = [-0.015, -0.015, 0.013921]
    ...
    for i in range(len(timeStep)):
        ...
        # Rescales temputure coloring gradient
...
# This function reads csv files and puts in the data into arrays
def readCsv():
    # Csv file paths
    ...
    with open(xCsv, "r") as csvfile:
        ...
        for row in csvreader:
            # Stores time values in timeValues array
            timeValues.append(float(row[0]))
            # Stores x velocity values in xVel array
            xVel.append(float(row[1]))
    with open(yCsv, "r") as csvfile:
        csvreader = csv.reader(csvfile)
        for row in csvreader:
            # Stores y velocity values in yVel array
            yVel.append(float(row[1]))
    with open(pCsv, "r") as csvfile:
        csvreader = csv.reader(csvfile)
        for row in csvreader:
            # Stores power values in power array
            power.append(float(row[1]))


# This function generates the x position values for every timestep
# This funtion uses the xVel array to determine when the laser is moving and how long it is moving for
# Since this function is used for generating positions for both camera and clip, this function takes in the
# the intitial position as initialPos and the array for the positions to be stored in ar xArr
def createXPos(speed, initialPos, xArr):
    arrFull = False
    if len(timeStep) > 1:
        arrFull = True

    # Change in time between timestep
    dt = 0.00002
    prevPos = initialPos
    xArr.append(prevPos)
